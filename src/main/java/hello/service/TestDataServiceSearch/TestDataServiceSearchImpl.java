...
@Service("testDataServiceSearch")
public class TestDataServiceSearchImpl<V> implements TestDataServiceSearch<V> {
...
////////////^SEARCH собственно сам поиск с сортировкой
    ...
    public PageReturnFormat<V> criteriaSearchTestData(Integer page, Integer limit,
                                                      String[] searchParams, String[] sortByParams, String phone,
        ...
        //добавляем поиск по полю phone из таблицы TestDataInfo
        if (!(phone == null || phone.equals(""))) {
            predicate = addSearchByPhoneToPredicateAndJoinTableWithPhone(builder, root, predicate, phone);
        }
...
//^ADD SEARCH BY PHONE
    public Predicate addSearchByPhoneToPredicateAndJoinTableWithPhone(CriteriaBuilder builder, Root root, Predicate predicate,
                                                                      String phone) {
        Join<TestData, TestDataInfo> joinTestDataInfo = null;


        if (!(phone == null || phone.equals(""))) {
            joinTestDataInfo = root.join("testDataInfoSet", JoinType.LEFT);
            predicate = builder.and(predicate,
                    builder.like(builder.lower(joinTestDataInfo.get("phone").as(String.class)),
                            ("%" + phone + "%").toLowerCase()));
        }
        return predicate;
    }
